...
export async function createOne(req, res) {
    const { title, short_description, description, wages, contract_type, idcompany } = req.body
    try {
        if (!title || !short_description || !description || !wages || !contract_type || !idcompany) {
            return res.status(400).json({ message: 'Title, short_description, description, wages, contract_type, idcompany needed.'})
        }
        const newone = await createOneThing(title, short_description, description, wages, contract_type, idcompany)
...
export async function modifyOne(req, res) {
    ...
    try{
        ...
        const updatedone = await modifyOneThing(title, short_description, description, wages, contract_type, idcompany, id)
        if (updatedone) {
